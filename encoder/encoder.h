...
public:
	...
	int imageGrayscaleAt_0_0, imageGrayscaleAt_0_1;  //固定16bits

	vector<int> mask;  //已废弃

	vector<pair<int, int>> pairs;  //每对8+8bits
	int toembedLength;  //  固定16bits
	...
	//这个函数在转换成员变量时会将新变量添加到字符前，因此decoder端先读到内容在这个函数里写在最后
	string AddInfo() {
		string extendInfo;
		...
		//转换imageGrayscaleAt_0_0,imageGrayscaleAt_0_1
